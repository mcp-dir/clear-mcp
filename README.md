...
### Clear no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **Clear** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do Clear em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **Clear via Open Finance Brasil**, regulado pelo Banco Central
...
[➕ Abrir no Claude e conectar](https://claude.ai/customize/connectors?modal=add-custom-connector&mcpName=Clear%20MCP&mcpServerUrl=https%3A%2F%2Fapi.mcp.ai%2Fp_clear)

**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Clear MCP` e **URL** `https://api.mcp.ai/p_clear`.

### Cursor

[➕ Instalar Clear MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=clear&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9jbGVhciJ9)

### VS Code (Copilot Chat)

[➕ Instalar Clear MCP no VS Code](vscode:mcp/install?name=clear&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_clear%22%7D)
...
```
Quanto gastei no cartão do Clear esse mês?
Resuma minha fatura do Clear em aberto
Qual meu saldo na conta do Clear?
Como está minha carteira de investimentos no Clear?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Clear com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Clear?**
Este MCP é focado no Clear. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [clear@mcp.ai](mailto:clear@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/clear-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_clear` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
